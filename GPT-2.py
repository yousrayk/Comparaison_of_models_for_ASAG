...
if __name__ == '__main__':
    ...
    for stu_ans in student_answers:
        ...
        word_array_1 = embedder.gpt2_embedding(pp_desired)
        word_array_2 = embedder.gpt2_embedding(pp_student)
        ...
        text_1_embed = embedding_operations.gpt_sowe(word_array_1)
        text_2_embed = embedding_operations.gpt_sowe(word_array_2)

        gpt2_similarity_scores[stu_ans] = similarity_metrics.get_cosine_similarity(text_1_embed, text_2_embed)

    print(gpt2_similarity_scores)

    # Save similarity scores to JSON
    with open('gpt2_similarity_scores.json', 'w') as fp:
        json.dump(gpt2_similarity_scores, fp)

    # Update DataFrame with the similarity scores
    for answer in student_answers:
        df.loc[df['student_answer'] == answer, 'gpt2_sim_score'] = gpt2_similarity_scores[answer]
